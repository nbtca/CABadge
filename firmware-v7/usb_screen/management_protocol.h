...
enum { MG_WIFI_ENABLE=1, MG_WIFI_SCAN, MG_WIFI_CONNECT, MG_WIFI_DISCONNECT,
       ...
       MG_BRIGHTNESS, MG_SLEEP, MG_REDUCED };
typedef struct { uint8_t token[16]; uint32_t generation; uint64_t deadline; bool open; } mg_session_t;
static inline void mg_close(mg_session_t *s){s->open=false;s->deadline=0;memset(s->token,0,16);s->generation++;}
static inline void mg_open(mg_session_t *s,const uint8_t token[16],uint64_t now){
    mg_close(s);memcpy(s->token,token,16);s->deadline=now+MG_WINDOW_MS;s->open=true;
}
static inline bool mg_active(const mg_session_t *s,uint64_t now){return s->open&&now<s->deadline;}
static inline bool mg_authorized(const mg_session_t *s,const uint8_t token[16],uint64_t now){
    unsigned diff=0;for(int i=0;i<16;i++)diff|=s->token[i]^token[i];return mg_active(s,now)&&!diff;
}
static inline int mg_unhex(char c){return c>='0'&&c<='9'?c-'0':c>='a'&&c<='f'?c-'a'+10:c>='A'&&c<='F'?c-'A'+10:-1;}
static inline bool mg_parse_key(const char *s,uint8_t out[16]){
    if(strlen(s)!=32)return false;
    for(int i=0;i<16;i++){int a=mg_unhex(s[i*2]),b=mg_unhex(s[i*2+1]);if(a<0||b<0)return false;out[i]=(a<<4)|b;}return true;
}
