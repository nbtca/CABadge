...
static int xSemaphoreTake(SemaphoreHandle_t s,unsigned ticks){(void)ticks;if(!s||s->held)return 0;s->held=true;return 1;}
...
static uint8_t mock_flash[0x100000];static esp_partition_t mock_partition={sizeof(mock_flash)};
...
typedef struct {char ssid[33];int rssi;unsigned security;} badge_wifi_ap_t;
...
static int httpd_req_recv(httpd_req_t *r,char *out,size_t n){
    ...
    if(n>r->content_len-r->offset)n=r->content_len-r->offset;memcpy(out,r->body+r->offset,n);r->offset+=n;return (int)n;
}
